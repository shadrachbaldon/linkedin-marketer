...
function updateValues(name, value){
    ...
    dbrequest.onupgradeneeded = function(event) {
		...
		// CONTINUE HERE GIVING INITIAL VALUES TO MESSAGE SETTINGS TABLE
    }
    ...
    function saveSetting(settingData){
    	...
    	dbrequest.onsuccess = function (e){
			...
			request.onsuccess = function(event) {
				// Get the old value that we want to update
				var data = event.target.result;
				console.log("Result from initial table: "+data);
				data.MessagePerPeriod = settingData.MessagePerPeriod;
				data.MessageHoursPerPeriod = settingData.MessageHoursPerPeriod;
				data.Message = settingData.Message;
				data.Mode = settingData.Mode;
				data.Recipients = settingData.Recipients;
				data.Continue = settingData.Continue;
				data.LastIndex = settingData.LastIndex;
				// update the value(s) in the object that you want to change
			  	
			 	console.log("save setting data retrieved:");
			 	console.log(data);
			...
			if (totalListItems > 0) {
				...
				getCurrentMsgSettings(function(result){
					...
					if (index < 0) {
						...
						url = `https://www.linkedin.com/messaging/compose/?after=profile.view&recipient=${list[index].id}`;
						data.LastIndex = index+1;
						focusForm();
						typeMsg(result.Message);
						focusForm();
						typeMsg(result.Message);
						focusForm();
						// clickSendBtn();
